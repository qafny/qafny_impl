...
class ProgramTransformer(ExpVisitor):
    ...
    def attachContext(self, node: QXTop, antlr_node: antlr4.ParserRuleContext):
        '''Helper method to set the line, column and range of a node.'''
    ...
    def visitProgram(self, ctx: ExpParser.ProgramContext):
        ...
        while ctx.topLevel(i) is not None:
            ...
            i = i + 1
        return QXProgram(topLevelStmts, parser_context=ctx)
    ...
    def visitTopLevel(self, ctx: ExpParser.TopLevelContext):
        if ctx.TInclude() is not None:
            ...
            return QXInclude(path, parser_context=ctx)
        else:
    ...
    def visitMethod(self, ctx: ExpParser.MethodContext):
        ...
        if not axiom:
            ...
            stmts = []
        return QXMethod(ctx.ID(), axiom, bindings, returns, conds, stmts, parser_context=ctx)
    ...
    def visitFunction(self, ctx: ExpParser.FunctionContext):
        ...
        return QXFunction(ctx.ID(), axiom, bindings, return_type, body, parser_context=ctx)
    ...
    def visitLemma(self, ctx: ExpParser.LemmaContext):
        ...
        if ctx.Axiom() is not None:
            ...
            axiom = False
        conds = self.visitConds(ctx.conds())
        return QXLemma(ctx.ID(), axiom, bindings, conds, parser_context=ctx)
    ...
    def visitPredicate(self, ctx: ExpParser.PredicateContext):
        ...
        return QXPredicate(ctx.ID(), bindings, arith_expr, parser_context=ctx)
    ...
    def visitConds(self, ctx: ExpParser.CondsContext):
        ...
        while ctx.spec(i) is not None:
            ...
            if top == 'requires':
                conds.append(QXRequires(spec, parser_context=ctx))
            elif top == 'ensures':
                conds.append(QXEnsures(spec, parser_context=ctx))

            i = i + 1

        # convert decreases
        for a_exp in ctx.arithExpr():
            conds.append(QXDecreases(self.visitArithExpr(a_exp), parser_context=ctx))
    ...
    def visitLoopConds(self, ctx: ExpParser.LoopCondsContext):
        ...
        i = 0
        while ctx.spec(i) is not None:
            conditions.append(QXInvariant(self.visitSpec(ctx.spec(i)), parser_context=ctx))
            i += 1

        i = 0
        while ctx.arithExpr(i) is not None:
            conditions.append(QXDecreases(self.visitArithExpr(ctx.arithExpr(i)), parser_context=ctx))
            i += 1

        i = 0
        while ctx.locus(i) is not None:
            conditions.append(QXSeparates(self.visitLocus(ctx.locus(i)), parser_context=ctx))
            i += 1
    ...
    def visitQbool(self, ctx: ExpParser.QboolContext):
        if ctx.qbool() is not None:
            ...
            return QXQNot(v, parser_context=ctx)
        if ctx.idindex() is not None:
            ...
            return QXQComp(op, left, right, index, parser_context=ctx)
    ...
    def visitLogicImply(self, ctx: ExpParser.LogicImplyContext):
        if ctx.logicImply() is not None:
            ...
            return QXLogic("==>", v1, v2, parser_context=ctx)
    ...
    def visitAllspec(self, ctx: ExpParser.AllspecContext):
        if ctx.chainBExp() is not None:
            ...
            return QXAll(bind, bounds, imply, parser_context=ctx)
            ...
            bounds = QXComp("<=", crange.left(), QXComp("<", QXBind(bind.ID(), parser_context=ctx), crange.right(), parser_context=ctx), parser_context=ctx.crange())
    ...
    def visitLogicExpr(self, ctx: ExpParser.LogicExprContext):
        if len(ctx.logicExpr()) > 0:
            ...
            if ctx.getChild(0).getText() == 'not':
                return QXCNot(self.visitLogicExpr(ctx.logicExpr(0)), parser_context=ctx)
            return QXLogic(ctx.getChild(1), self.visitLogicExpr(ctx.logicExpr(0)), self.visitLogicExpr(ctx.logicExpr(1)), parser_context=ctx)
        elif ctx.chainBExp() is not None:
            return self.visitChainBExp(ctx.chainBExp())
    ...
    def visitChainBExp(self, ctx: ExpParser.ChainBExpContext):
        ...
        for i in range(len(op)):
            result = QXComp(op[i], result, va[i+1], parser_context=ctx)
        return result

    # Visit a parse tree produced by ExpParser#logicInExpr.
    def visitLogicInExpr(self, ctx: ExpParser.LogicInExprContext):
        # right contains left
        return QXBin('∈', self.visitArithExpr(ctx.arithExpr(0)), self.visitArithExpr(ctx.arithExpr(1)), parser_context=ctx)
    ...
    def visitQunspec(self, ctx: ExpParser.QunspecContext):
        ...
        return QXQSpec(locus, qty, states, parser_context=ctx)
    ...
    def visitQspec(self, ctx: ExpParser.QspecContext):
        ...
        if ctx.manyketpart() is not None:
            if ctx.arithExpr() is not None:
                return QXTensor(self.visitManyketpart(ctx.manyketpart()), None, None, self.visitArithExpr(ctx.arithExpr()), parser_context=ctx)
            else:
    ...
    def visitPartspec(self, ctx: ExpParser.PartspecContext):
        ...
        if len(ctx.arithExpr()) == 4:
            ...
            return QXPart(num, fname, true, false, parser_context=ctx)
            ...
            return QXPartWithPredicates(size, true_predicate, false_predicate, parser_context=ctx)
        elif ctx.boolLiteral() is not None:
            # part(ID, boolLiteral, arithExpr)
            boolLit = self.visitBoolLiteral(ctx.boolLiteral())
            amplitude = self.visitArithExpr(ctx.arithExpr(0))
            return QXPartGroup(ctx.ID(), boolLit, amplitude, parser_context=ctx)
            ...
            return QXPartLambda(predicate, amplitude, parser_context=ctx)
        elif ctx.partsections() is not None:
            # part(partsections)
            return QXPartWithSections(self.visitPartsections(ctx.partsections()), parser_context=ctx)
        else:
    ...
    def visitPartpred(self, ctx: ExpParser.PartpredContext):
        ...
        return QXPartPredicate(amplitude, predicate, parser_context=ctx)
    ...
    def visitPartsection(self, ctx: ExpParser.PartsectionContext):
        ...
        return QXPartsection(amplitude, ket, predicate, parser_context=ctx)
    ...
    def visitSumspec(self, ctx: ExpParser.SumspecContext):
                ...
                if expr.op() == '*':
                    ...
                    if isinstance(expr.right(), QXBExp):
                        ...
                        if condition is not None:
                            ...
                            condition = QXLogic('&&', expr.left(), expr.right(), parser_context=ctx)
                            tree = QXNum(1, parser_context=ctx)
                        else:
                    ...
                    if condition is None:
                        ...
                        if tree_condition is not None:
                            ...
                            condition = QXLogic('&&', condition, tree_condition, parser_context=ctx)
                    ...
                    if isinstance(expr.left(), QXBExp):
                        ...
                        return expr.left(), QXBin('/', QXNum(1, parser_context=ctx), expr.right(), parser_context=ctx)
                    else:
                        condition, left_expr = extract_condition(expr.left())
                        # need to repair the QXAExp if the condition was modified
                        if condition is not None:
                            return (condition, QXBin('/', left_expr, expr.right(), parser_context=ctx))
                        else:
                            return (None, expr)
                else:
        ...
        if ctx.manyketpart() is not None:
            ...
            return QXSum([sum], amp, kets, parser_context=ctx)
            ...
            if amp is not None:
                amp = QXBin('*', amp, next_sum.amp(), parser_context=ctx)
            else:
                amp = next_sum.amp()
            ...
            return QXSum(sums, amp, next_sum.kets(), parser_context=ctx)
    ...
    def visitMaySum(self, ctx: ExpParser.MaySumContext):
        return QXCon(ctx.ID(), self.visitCrange(ctx.crange()), self.visitBexp(ctx.bexp()), parser_context=ctx)

    # Visit a parse tree produced by ExpParser#asserting.
    def visitAsserting(self, ctx: ExpParser.AssertingContext):
        return QXAssert(self.visitSpec(ctx.spec()), parser_context=ctx)
    ...
    def visitCasting(self, ctx: ExpParser.CastingContext):
        ...
        return QXCast(qty, locus, parser_context=ctx)
    ...
    def visitVarcreate(self, ctx: ExpParser.VarcreateContext):
        ...
        for binding in bindings:
            stmts.append(QXInit(binding, parser_context=ctx))

        if ctx.arithExpr() is not None:
            value = self.visitArithExpr(ctx.arithExpr())
            stmts.append(QXCAssign(bindings, value, parser_context=ctx))

        return stmts

    # Visit a parse tree produced by ExpParser#assigning.
    def visitAssigning(self, ctx: ExpParser.AssigningContext):
        return QXCAssign(self.visitIdindices(ctx.idindices()), self.visitArithExpr(ctx.arithExpr()), parser_context=ctx)
    ...
    def visitQassign(self, ctx: ExpParser.QassignContext):
        ...
        if ctx.locus() is not None:
            ...
            location = QXBind(ctx.ID(), parser_context=ctx)
        exp = self.visitExpr(ctx.expr())
        return QXQAssign(location, exp, parser_context=ctx)
    ...
    def visitQcreate(self, ctx: ExpParser.QcreateContext):
        ...
        return QXQCreate(qrange, size, parser_context=ctx)
    ...
    def visitMeasure(self, ctx: ExpParser.MeasureContext):
        ...
        stmts.append(QXMeasure(assign_to, locus, restrict, parser_context=ctx))
        return stmts
    ...
    def visitIfexp(self, ctx: ExpParser.IfexpContext):
        ...
        return QXIf(bexp, stmts, else_stmts, parser_context=ctx)
    ...
    def visitCifexp(self, ctx: ExpParser.CifexpContext):
        ...
        return QXIfExp(b, l, r, parser_context=ctx)
    ...
    def visitForexp(self, ctx: ExpParser.ForexpContext):
        ...
        if ctx.bexp() is not None:
            ...
            body = [QXIf(control, body, None, parser_context=ctx)]
        return QXFor(id, crange, inv, body, parser_context=ctx)
    ...
    def visitWhileexp(self, ctx: ExpParser.WhileexpContext):
        ...
        return QXWhile(bexp, invs, stmts, parser_context=ctx)
    ...
    def visitFcall(self, ctx: ExpParser.FcallContext):
        ...
        if ctx.getChild(1) is not None and ctx.getChild(1).getText() == '^{-1}':
            inverse = True
        return QXCall(ctx.ID(), self.visitArithExprsOrKets(ctx.arithExprsOrKets()), inverse, parser_context=ctx)
    ...
    def visitArithExprWithSum(self, ctx: ExpParser.ArithExprWithSumContext) -> QXAExp:
        if len(ctx.arithExprWithSum()) == 2:
            ...
            return QXBin(op, left, right, parser_context=ctx)
        elif ctx.maySum() is not None:
            summation = self.visitMaySum(ctx.maySum())
            aexp = self.visitArithExprWithSum(ctx.arithExprWithSum(0))
            return QXSumAExp(summation, aexp, parser_context=ctx)
    ...
    def visitArithExpr(self, ctx: ExpParser.ArithExprContext):
        if ctx.cifexp() is not None:
            ...
            return QXBin(op, v1, v2, parser_context=ctx)
        elif ctx.arithAtomic() is not None:
            return self.visitArithAtomic(ctx.arithAtomic())
        else:
    ...
    def visitArithAtomic(self, ctx: ExpParser.ArithAtomicContext):
        if ctx.numexp() is not None:
            ...
            return QXNegation(self.visitArithAtomic(ctx.arithAtomic()), parser_context=ctx)
        elif ctx.boolLiteral() is not None:
            return self.visitBoolLiteral(ctx.boolLiteral())
        elif ctx.arithExpr() is not None:
            return self.visitArithExpr(ctx.arithExpr())
    ...
    def visitUniCall(self, ctx: Union[ExpParser.SinExprContext, ExpParser.CosExprContext, ExpParser.SqrtExprContext]):
        ...
        if ctx.arithAtomic() is not None:
            return QXUni(fname, self.visitArithAtomic(ctx.arithAtomic()), parser_context=ctx)
        elif ctx.arithExpr() is not None:
            # regular function call (i.e. sin(a))
            fcall = QXUni(fname, self.visitArithExpr(ctx.arithExpr()), parser_context=ctx)
            # check for exponent
            if ctx.numexp() is not None:
                fcall = QXBin("^", sin_expr, QXNum(int(ctx.numexp().getText())), parser_context=ctx)
            return fcall
    ...
    def visitNotExpr(self, ctx: ExpParser.NotExprContext):
        return QXUni("not", self.visitArithExpr(ctx.arithExpr()), parser_context=ctx)

    # Visit a parse tree produced by ExpParser#absExpr.
    def visitAbsExpr(self, ctx: ExpParser.AbsExprContext):
        return QXUni("abs", self.visitArithExpr(ctx.arithExpr()), parser_context=ctx)
    ...
    def visitRotExpr(self, ctx: ExpParser.RotExprContext):
        return QXUni("rot", self.visitArithExpr(ctx.arithExpr()), parser_context=ctx)

    # Visit a parse tree produced by ExpParser#ketCallExpr.
    def visitKetCallExpr(self, ctx: ExpParser.KetCallExprContext):
        return QXUni("ket", self.visitArithExpr(ctx.arithExpr()), parser_context=ctx)
    ...
    def visitLambdaT(self, ctx: ExpParser.LambdaTContext):
        ...
        return QXOracle(bindings, amplitude_expr, kets, inverse, parser_context=ctx)
    ...
    def visitKet(self, ctx: ExpParser.KetContext):
        ...
        if len(ctx.qstate()) > 0:
            ...
            for qstate in ctx.qstate():
                kets.append(QXSKet(self.visitQstate(qstate), ctx.TSub() is not None, parser_context=qstate))
            return kets
        elif ctx.arithAtomic() is not None:
            return [QXVKet(self.visitArithAtomic(ctx.arithAtomic()), parser_context=ctx)]

    # Visit a parse tree produced by ExpParser#ketsum.
    def visitKetsum(self, ctx: ExpParser.KetsumContext):
        return QXSumAExp(self.visitMaySum(ctx.maySum()), self.visitArithExpr(ctx.arithExpr()), parser_context=ctx)
    ...
    def visitQstate(self, ctx: ExpParser.QstateContext):
        if ctx.arithExpr() is not None:
            ...
            return QXHad(self.visitAdditiveOp(ctx.additiveOp()), parser_context=ctx)
    ...
    def visitBinding(self, ctx: ExpParser.BindingContext):
        ...
        return QXBind(id, t, parser_context=ctx)
    ...
    def visitTypeOptionalBinding(self, ctx: ExpParser.TypeOptionalBindingContext):
        ...
        return QXBind(ctx.ID(), type, parser_context=ctx)
    ...
    def visitCrange(self, ctx: ExpParser.CrangeContext):
        return QXCRange(self.visitArithExpr(ctx.arithExpr(0)), self.visitArithExpr(ctx.arithExpr(1)), parser_context=ctx)
    ...
    def visitIdindex(self, ctx: ExpParser.IdindexContext):
        return QXQIndex(ctx.ID(), self.visitIndex(ctx.index()), parser_context=ctx)
    ...
    def visitQrange(self, ctx: ExpParser.QrangeContext):
        ...
        if index is not None and crange is None:
            ...
            if isinstance(index, QXNum):
                crange = QXCRange(index, QXNum(index.num() + 1, parser_context=ctx), parser_context=ctx.index())
                ...
                if next_num != 0:
                    ...
                    crange = QXCRange(index, QXBin(op, index.left(), QXNum(abs(next_num), parser_context=ctx), parser_context=ctx), parser_context=ctx.index())
                else:
                    # for the second one (upper bound), we can ignore the summand
                    crange = QXCRange(index, index.left(), parser_context=ctx.index())
            else:
                crange = QXCRange(index, QXBin("+", index, QXNum(1), parser_context=ctx), parser_context=ctx.index())

            index = None

        return QXQRange(location, index, crange, parser_context=ctx)

    # Visit a parse tree produced by ExpParser#numexp.
    def visitNumexp(self, ctx: ExpParser.NumexpContext):
        return QXNum(utils.str_to_num(ctx.getText()), parser_context=ctx)
    ...
    def visitTypeT(self, ctx: ExpParser.TypeTContext):
        ...
        if ctx.typeT() is not None:
            ...
            return TyFun(types, self.visitTypeT(ctx.typeT()), parser_context=ctx)
    ...
    def visitBaseTy(self, ctx: ExpParser.BaseTyContext):
        ...
        if isinstance(ctx, ExpParser.NaturalTypeContext):
            return TySingle("nat", parser_context=ctx)
        elif isinstance(ctx, ExpParser.RealTypeContext):
            return TySingle("real", parser_context=ctx)
        elif isinstance(ctx, ExpParser.IntTypeContext):
            return TySingle("int", parser_context=ctx)
        elif isinstance(ctx, ExpParser.BoolTypeContext):
            return TySingle("bool", parser_context=ctx)
        elif isinstance(ctx, ExpParser.BitVectorTypeContext):
            return TySingle(ctx.TBV().getText(), parser_context=ctx)
        elif isinstance(ctx, ExpParser.DynamicArrayTypeContext):
            return TyArray(ctx.baseTy().accept(self), None, parser_context=ctx)
        elif isinstance(ctx, ExpParser.SetTypeContext):
            return TySet(ctx.baseTy().accept(self), parser_context=ctx)
            ...
            return TyArray(ty, v, parser_context=ctx)
        elif isinstance(ctx, ExpParser.QBitStringTypeContext):
            return TyQ(self.visitArithExpr(ctx.arithExpr()), parser_context=ctx)
        else:
    ...
    def visitQty(self, ctx: ExpParser.QtyContext):
        if ctx.Nor() is not None:
            return TyNor(parser_context=ctx)
        if ctx.Had() is not None:
            return TyHad(parser_context=ctx)
        if ctx.En() is not None:
            if ctx.arithExpr() is not None:
                return TyEn(self.visitArithExpr(ctx.arithExpr()), parser_context=ctx)
            return TyEn(QXNum(1, parser_context=ctx), parser_context=ctx)
    ...
    def visitBoolLiteral(self, ctx: ExpParser.BoolLiteralContext):
        if ctx.getText() == 'true' or ctx.getText() == 'True':
            return QXBoolLiteral(True, parser_context=ctx)
        elif ctx.getText() == 'false' or ctx.getText() == 'False':
            return QXBoolLiteral(False, parser_context=ctx)
        else:
