datatype Nor = Nor(k: bv1)
datatype Had = Had(0: bv1, 1: bv1)
...
function alpha(n: nat, f: nat -> bv1): (En1 -> real)
    requires n >= 0
    ensures forall state :: alpha(n, f)(state) == if state.k < pow2(n) && f(state.k) == 1 then
...
function beta(n: nat, f: nat -> bv1): (En1 -> real)
    requires n >= 0
    ensures forall state :: beta(n, f)(state) == if state.k < pow2(n) && f(state.k) == 0 then
        (1.0/sqrt(pow2(n) as real)) * a(state.k) 
      else if state.k < pow2(n) && f(state.k) == 0 then
        0.0
      else 
...
function phi(n: nat, alpha: (En1 -> real), beta: (En1 -> real), theta: real): (En1 -> real)
    requires n >= 0
    ensures forall state :: phi(n, alpha, beta, theta)(state) == if 0 <= state.k < pow2(n) then 
            sin(theta)*alpha(state) + cos(theta)*beta(state)
             else 0.0



//properties of t unitary operation
function VU(alpha: (En1 -> real), beta: (En1 -> real), theta: real, n: nat, t: nat): (En1 -> real)
