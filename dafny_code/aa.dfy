...
method amplitude(x:seq<seq<bv1>>, n: nat) returns (cx: nat, xa: real, cy: nat, ya: real) //x is hadtype. secret a, unknow to the algorithm
    requires |x| == n
    ensures cx + cy == |x|
    ensures xa*xa*cx as real + ya*ya*cy as real == 1.0
{
    ...
    cx := count;
    xa := 1.0/sqrt(count as real);
    cy := |x| - count;
    ya := 1.0/sqrt((|x| - count) as real);
    return cx, xa, cy, ya;
}
